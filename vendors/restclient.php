...
class restclient {
    ...
    private function _exec()
    {
        ...
        /* if ($this->httperror || $use_time > 3) {
          $trace = debug_backtrace();
          file_put_contents(__DIR__ . '/RestClient.log', date('Y-m-d H:i:s')
          . "\nError:" . $this->httperror . "\nRestClient:" . print_r($this, TRUE)
          . "\nTrace[0]:" . print_r($trace[0], TRUE)
          . "\nTrace[1]:" . (!empty($trace[1]) ? print_r($trace[1], TRUE) : '')
          . "\nTrace[2]:" . (!empty($trace[2]) ? print_r($trace[2], TRUE) : '')
          . "\nTrace[3]:" . (!empty($trace[3]) ? print_r($trace[3], TRUE) : '')
          . "\n\n\n", FILE_APPEND);
          } */
    }
